...
def test_rename_logic_with_author_override(
    media_test_folder: tuple[Path, list[str], list[str]], capsys
):
    """
...
def test_rename_logic_with_metadata_author(
    media_test_folder: tuple[Path, list[str], list[str]], capsys
):
    """
    ...
    for item in test_folder.iterdir():
        ...
        assert filename_pattern.match(
            item.name
        ), f"Filename '{item.name}' did not match expected pattern."
